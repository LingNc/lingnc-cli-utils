...
func main() {
	...
	// 定义命令行参数
	extractPath := flag.String("x", "", "解包模式：输入 .bak 文件 / 目录 / .zip (例如: -x backup.bak)")
	...
	useZip := flag.Bool("z", false, "打包模式：生成 .zip 归档文件")
	...
	if *extractPath != "" {
		bakPath, resolvedXML, cleanup, err := resolveInput(*extractPath)
		if err != nil {
			fmt.Printf("解包失败: %v\n", err)
			os.Exit(1)
		}
		...
		// 执行解包
		if err := extractBackup(bakPath, resolvedXML); err != nil {
			fmt.Printf("解包失败: %v\n", err)
			os.Exit(1)
		}
	} else if *packDir != "" {
		...
		cleanupTemp := func() {}
		if *useZip {
			tmpDir, err := os.MkdirTemp("", "miui_pack_")
			if err != nil {
				fmt.Printf("打包失败: %v\n", err)
				os.Exit(1)
			}
			outputBaseDir = tmpDir
			cleanupTemp = func() { _ = os.RemoveAll(tmpDir) }
		}

		outputDir, err := createBackup(*packDir, *headPath, *xmlPath, outputBaseDir)
		if err != nil {
			fmt.Printf("打包失败: %v\n", err)
			os.Exit(1)
		}
		if *useZip {
			defer cleanupTemp()
			zipPath := filepath.Base(outputDir) + ".zip"
			fmt.Printf("正在生成 Zip 容器: %s\n", zipPath)
			if err := zipDirRecursively(outputDir, zipPath); err != nil {
				fmt.Printf("Zip 打包失败: %v\n", err)
				os.Exit(1)
			}
		}
	}
...
func printUsage() {
	...
	fmt.Println("用法:")
	fmt.Println("  解压: miui-backup -x <.bak 文件 | 目录 | .zip>")
	fmt.Println("  打包: miui-backup -c <文件夹路径> [-conf head.json] [-xml descript.xml] [-z]")
}
...
func extractBackup(filename, xmlPath string) error {
	...
	if info != nil && info.PackageName != "" {
		...
		if resolvedXML == "" {
			...
			candidate := filepath.Join(bakDir, "descript.xml")
			if _, err := os.Stat(candidate); err == nil {
				resolvedXML = candidate
			}
		}
		if resolvedXML != "" {
			_ = copyFile(resolvedXML, filepath.Join(appsDir, "descript.xml"))
		}
	}
...
func createBackup(sourceDir, headPath, xmlPath, outputBaseDir string) (string, error) {
	...
	if strings.TrimSpace(outputBaseDir) != "" {
		outDir = filepath.Join(outputBaseDir, appLabel)
	}
